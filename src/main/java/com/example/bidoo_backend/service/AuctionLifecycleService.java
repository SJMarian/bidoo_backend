        ...
        for (AuctionItem item : items) {
            ...
            if (item.getStartAt() != null && item.getEndAt() != null) {
                if (now.isBefore(item.getStartAt())) {
                    item.setStatus(AuctionItemStatus.UPCOMING);
                } else if (!now.isBefore(item.getStartAt()) && now.isBefore(item.getEndAt())) {
                    item.setStatus(AuctionItemStatus.ACTIVE);
                } else if (!now.isBefore(item.getEndAt())) {
                    item.setStatus(AuctionItemStatus.CLOSED);
                }
